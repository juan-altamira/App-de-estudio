...
- implemented social gate via `UsageStatsManager` + persistent foreground monitor, with the single Activity reimposed while unresolved and no `AccessibilityService` or `TYPE_APPLICATION_OVERLAY`; permanently active across supported social apps without an off toggle or inactive state, boot-persistence of active gate sessions across device reboots, per-app settings with daily frequency and time window, per-app daily quota tracking, pending-backed unlock progress (`0/N` while locked), direct reuse/open of the real `Tarjetas pendientes` QUICK session, no blocking when there is no real pending debt, no gate-only filler questions, a foreground-only escape token with 2 uses per rolling 7-day window and visible remaining uses, and a legacy real-only `SOCIAL_GATE` scheduler path kept for compatibility
...
- must render inside the existing single Activity; it must not add a second Activity or a
  `TYPE_APPLICATION_OVERLAY` window
- while unresolved and the display is usable, losing Activity focus must cause the foreground
  monitor to reimpose that Activity; calls and the lock screen keep priority
- the special-access permission may be retained solely for Android background-Activity launch
  eligibility; merely holding it must not produce an alert-window surface
- must remain permanently enabled for all supported social apps; the UI, ViewModel and repository must not provide an off toggle or inactive state for supported social apps
...
- the manual course builder may save each unit as soon as it has at least one complete question and no incomplete questions; it must not require four strict friction-1 questions
- when the manual builder targets an existing unit, it must compile the new questions into a fresh additive node/outcome batch, preserve the existing course/unit metadata, and never overwrite, archive or delete existing questions or their progress
- a normal manual `Con tus palabras` question compiles as real `IN_APP_QUICK` plus `IN_APP_DEEP` debt, so it appears in `Tarjetas pendientes`; it remains outside auto-graded external surfaces
